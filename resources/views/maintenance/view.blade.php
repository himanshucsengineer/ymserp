...
function filterByDate(){
    ...
    $.ajax({
        type: "post",
        url: "/api/maintenance/filterByDateRepair",
        headers: {
            'Authorization': 'Bearer ' + checkToken
        },
        data:{
...
function refreshTable(page,search){
    ...
    if(page){
        url = `/api/maintenance/getInspectionDataRepair?page=${page}`;
    }else if(search){
        url = `/api/maintenance/getInspectionDataRepair?search=${search}`;
    }else{
        url= `/api/maintenance/getInspectionDataRepair`;
    }
